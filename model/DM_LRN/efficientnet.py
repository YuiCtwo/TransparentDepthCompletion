...
class EfficientNet(_EfficientNet):
    def __init__(self, arch, multi_scale_output=True):

        blocks_args, global_params = get_model_params(arch, dict(image_size=None))
        super().__init__(blocks_args, global_params)

        self.multi_scale_output = multi_scale_output
        self.stage_specs = sys.modules[__name__].__getattribute__(arch.replace("-", "_"))
