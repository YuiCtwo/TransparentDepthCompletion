...
class RealTimeCaptureDataset(Dataset):
    ...
    def __init__(self, root_dir, img_w=256, img_h=256,
                 mask_dir="mask", rgb_dir="rgb", depth_dir="depth_origin", depth_factor=4000) -> None:
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        pt = random_xyz_sampling(xyz_img, n_points=4800)

        if idx != len(self.img_list) - 1:
            forward_rgb = Image.open(self.img_list[idx+1]).convert("RGB")
            ...
            forward_mask = mask_loader(self.mask_list[idx+1])
            forward_mask = safe_resize(forward_mask, self.img_w, self.img_h)
            forward_depth = safe_resize(png_depth_loader(self.depth_list[idx+1]) / self.depth_factor, self.img_w, self.img_h)
